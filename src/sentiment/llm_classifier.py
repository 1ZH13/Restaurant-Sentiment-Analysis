"""
LLM-based aspect sentiment classifier for restaurant reviews.
Uses OpenAI API or Ollama for local inference.
"""
...
class AspectSentimentClassifier:
    """Classifier using LLM for aspect-based sentiment analysis."""
    ...
    def __init__(self, provider: str = "openai", model: str = "gpt-3.5-turbo",
        ...
        Args:
            ...
            api_key: API key
            base_url: Base URL for custom/ollama APIs (e.g., MiniMax, Ollama)
        """
    ...
    def analyze_review(self, review_text: str) -> SentimentResult:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that analyzes restaurant reviews."},
                    {"role": "user", "content": ASPECT_SENTIMENT_PROMPT.format(review_text=review_text)}
                ],
                temperature=0.0,
                max_tokens=200
            )
