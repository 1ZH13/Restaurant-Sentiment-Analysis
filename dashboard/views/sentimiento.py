...
def render(df: pd.DataFrame):
    ...
    st.markdown("### Resumen de distribución de sentimiento")
    ...
    positive_pct = (sentiment_counts.get("Positivo", 0) / total * 100) if total > 0 else 0
    negative_pct = (sentiment_counts.get("Negativo", 0) / total * 100) if total > 0 else 0
    ...
    with col1:
        ...
        <div style="background-color: rgba(40, 167, 69, 0.15); padding: 16px; border-radius: 12px; border-left: 4px solid #28a745;">
            ...
            <p style="margin: 4px 0 0 0; color: #A0AEC0;">Positivo</p>
        </div>
    ...
    with col3:
        ...
        <div style="background-color: rgba(220, 53, 69, 0.15); padding: 16px; border-radius: 12px; border-left: 4px solid #dc3545;">
            ...
            <p style="margin: 4px 0 0 0; color: #A0AEC0;">Negativo</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div style="background-color: #1E2530; padding: 16px; border-radius: 12px; border-left: 4px solid {score_color};">
            ...
            <p style="margin: 4px 0 0 0; color: #A0AEC0;">Puntaje prom.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if "category" in df.columns:
        st.markdown("---")
        st.markdown("### Mapa de calor de sentimiento por categoría")
        ...
        fig = px.imshow(
            ...
            x=aspect_cols,
            y=heatmap_df["Categoría"].values,
            color_continuous_scale="RdYlGn",
            range_color=[-1, 1],
            labels=dict(x="Aspecto", y="Categoría", color="Sentimiento"),
            text_auto=".2f"
        )
    ...
    with col1:
        st.markdown("#### Reseñas más positivas")

        positive_df = df[df["overall_sentiment"] > 0.3].nlargest(5, "overall_sentiment")

        for _, row in positive_df.iterrows():
            restaurant = row.get("restaurant_name", "Desconocido")
            ...
            <div style="background-color: rgba(40, 167, 69, 0.1); padding: 12px; border-radius: 8px; margin-bottom: 8px; border-left: 3px solid #28a745;">
                ...
                <span style="color: #A0AEC0; float: right;">Puntaje: {sentiment:.2f}</span>
                <p style="margin: 8px 0 0 0; color: #FAFAFA;">"{review}..."</p>
            </div>
            """, unsafe_allow_html=True)

    with col2:
        st.markdown("#### Reseñas más negativas")
