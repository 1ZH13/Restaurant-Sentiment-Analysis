...
def render(df: pd.DataFrame):
    ...
    if "cluster" not in df.columns or df["cluster"].isna().all():
        st.warning("El agrupamiento aún no se ha ejecutado. Corre `python run_pipeline.py`.")
        return
    ...
    st.caption("El agrupamiento (K-Means) se calcula por restaurante a partir de su "
               "calificación, sentimiento por aspecto, nivel de precio y volumen de reseñas.")
...
def _render_profiles(data: pd.DataFrame, cluster_ids) -> None:
    ...
    for cid in cluster_ids:
        ...
        row = {
            ...
            "Reseñas": len(subset),
            "Calificación": round(subset["overall_rating"].mean(), 2)
    ...
    st.caption("Los valores de sentimiento van de -1 a +1 y promedian solo las reseñas "
...
def _render_members(data: pd.DataFrame, cluster_ids) -> None:
    ...
    if subset.empty:
        st.info("Ningún restaurante coincide con esa búsqueda.")
        return
    ...
    members = (
        ...
        .rename(columns={"restaurant_name": "Restaurante", "rating": "Calificación",
                         "resenas": "Reseñas"})
        .sort_values("Calificación", ascending=False)
    )[["Restaurante", "Calificación", "Reseñas"]]
...
def _render_scatter(data: pd.DataFrame) -> None:
    ...
    fig = px.scatter(
        ...
        hover_data={"rating": ":.2f", "sentiment": ":.2f", "reseñas": True,
                    "Grupo": True, "cluster": False},
        color_discrete_sequence=CLUSTER_COLORS,
        labels={"rating": "Calificación promedio", "sentiment": "Sentimiento promedio"},
    )
    fig.update_layout(
        plot_bgcolor=TRANSPARENT, paper_bgcolor=TRANSPARENT,
        font=dict(color="#FAFAFA"), height=520,
        xaxis_title="Calificación promedio", yaxis_title="Sentimiento promedio",
        legend_title="Grupo", margin=dict(l=10, r=10, t=10, b=10),
    )
    st.plotly_chart(fig, width="stretch")
    st.caption("Cada punto es un restaurante; el tamano refleja cuantas reseñas tiene.")
